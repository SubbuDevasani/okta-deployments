...
# DEV_to_GIT_tfvars.py - ensure web apps have redirect_uris populated
...
def extract_oidc_tfvars(app: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if app_type not in ("web", "browser", "service", ""):
        return None
    application_type = "web" if app_type in ("web", "browser", "") else "service"
    ...
    token_endpoint_auth_method = cast(str, oc.get("token_endpoint_auth_method") or (
        "client_secret_basic" if application_type in ("web", "service") else "none"
    ))
